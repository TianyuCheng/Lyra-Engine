...
static bool process_stb(JSON& metadata, OSPath source_path, OSPath target_path)
{
    try {
        ...
        if (!pixels || width <= 0 || height <= 0) {
            ...
            get_logger()->error("Failed to load or invalid image file via STB: {}", source_path_str);
            return false;
        }
        ...
        get_logger()->error("Exception while processing STB image {}: {}", Path(source_path).string(), e.what());
        return false;
    } catch (...) {
        get_logger()->error("Unknown error while processing STB image {}", Path(source_path).string());
        return false;
    }
}
